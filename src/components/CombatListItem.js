...
const CombatListItem = ({ itemStyle="item-block", item, players, route, encounter, campaignId, adventureId, background }) => {
    ...
    useEffect(() => {
        ...
        if (item.enemy === "monster") {
            setdata({"name": monster.name, "cr": monster.cr, "type": monster.type}, console.log(data))
        } else if (item.enemy === "pc") {
            console.log("this is a player. set the data")
            setdata({"name": pc.name, "cr": pc.cr, "type": pc.type}, console.log(data))
        } else if (item.enemy === "npc") {
            setdata({"name": npc.name, "cr": npc.cr, "type": npc.type}, console.log(data))
        }
      return () => {
      }
    }, [])
